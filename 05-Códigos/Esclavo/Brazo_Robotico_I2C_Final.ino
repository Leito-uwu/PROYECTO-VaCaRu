...
bool emergencia = false;  // Emergencia activa o no
...
void moverServoGradual(Servo& servo, int& posActual, int destino, int paso, int tiempo_ms) {
  ...
  if (posActual < destino) {
    for (int pos = posActual; pos <= destino; pos += paso) {
      if (emergencia) return;  // Salir si hay emergencia
      servo.write(pos);
      delay(tiempo_ms);
    }
  } else {
...
void reposo() {
  ...
  moverServosParalelo(servo1, pos1, 60, servo2, pos2, 140, 1, 15);  // Garra y muñeca
...
void recogerBotella() {
  ...
  moverServoGradual(servo5, pos5, 172, 1, 10);                      // Girar base
...
void recibirDato(int byteCount) {
  if (Wire.available()) {
    ...
    //  Si es 3, activar emergencia
    if (comandoI2C == 3) {
      emergencia = true;
      Serial.println("Emergencia activada: Sistema detenido.");
    }
    // Si es 4, desactivar emergencia y regresar a reposo
    if (comandoI2C == 4) {
      emergencia = false;
...
void loop() {
  if (emergencia) {
    // Sistema en modo emergencia: NO HACER NADA
    return;
  }
  ...
  if (comandoNuevo) {
    ...
    if (comandoI2C == 0) {
      ...
      recogerBotella();
      depositarEnVerde();
    } else if (comandoI2C != 3 && comandoI2C != 4) {
